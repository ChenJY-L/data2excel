...
class GUI_Dialog(QWidget, QTUI.Ui_Data_Processing):
    ...
    def write_segment_points_sheet(self, wb, segment_points, C):
        ...
        headers = ['序号', '基准周期', '分段开始时间', '分段结束时间', '测量持续时间', '测量持续时间(min)'] if C else [
            'No.', 'Base Cycle', 'Start Time', 'End Time','Measurement segment time'
        ]
        ...
        for i, item in enumerate(segment_points, 1):
            rows.append([
                ...
                int(item['cycle']),
                float(item['time']),
                float(item['time'] + item['seg_minutes']),
                float(item['seg_minutes']),
                float(item['seg_minutes']*1440),
            ])

        ws.range(2, 1).value = rows
        if rows:
            ws.range(2, 3).resize(len(rows), 1).number_format = 'h:mm:ss'
            ws.range(2, 4).resize(len(rows), 1).number_format = 'h:mm:ss'
            ws.range(2, 5).resize(len(rows), 1).number_format = '[h]:mm:ss'
        ...
        series = chartApi.SeriesCollection().NewSeries()
        series.Name = f'={sheet_name}!$D$1'
        series.XValues = ws.range(f'$C$2:$C${len(rows) + 1}').api
        series.Values = ws.range(f'$E$2:$E${len(rows) + 1}').api
        series.MarkerStyle = 8
        series.MarkerSize = 5
        series.Format.Line.Weight = self.LINE_WEIGHT

        chartApi.HasTitle = True
        chartApi.ChartTitle.Text = '分段时间 vs 测量持续时间' if C else 'Segment Time vs Measurement Duration'
        chartApi.Axes(1).HasTitle = True
        chartApi.Axes(1).AxisTitle.Text = '分段时间' if C else 'Segment Time'
        chartApi.Axes(1).TickLabels.NumberFormatLocal = 'h:mm:ss'
        chartApi.Axes(2).HasTitle = True
        chartApi.Axes(2).AxisTitle.Text = '测量持续时间' if C else 'Measurement Duration'
        chartApi.Axes(2).TickLabels.NumberFormatLocal = '[h]:mm:ss'
